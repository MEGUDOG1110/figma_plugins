...
4. フレームごとにSVGの説明とリンク種別を確認する
5. 「選択フレームを書き出す」を押す

検出された対象はフレームごとに表示され、プラグインUIのラジオボタンから「URL」と「アンカー」を個別に切り替えられます。全体とフレーム単位の一括切り替えにも対応しています。選択した種別はFigmaファイル内に保存されます。

SVG全体の `aria-label` はプラグインUIからフレームごとに任意設定できます。SVGのネイティブなロールを維持するため、`role` 属性は追加しません。

複数フレームの書き出し時は、すべてのSVGを `svg-link-export.zip` にまとめて保存します。単一フレームの場合はSVGを直接保存します。
...
Figmaデスクトップ版の `Plugins > Development > Import plugin from manifest...` から、このディレクトリの `manifest.json` を読み込みます。
